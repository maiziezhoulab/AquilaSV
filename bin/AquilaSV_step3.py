...
parser.add_argument('--clean','-cl',type =int,help="You can choose to delete useless files or not, default = 1",default=1)
...
def main():
    if len(sys.argv) == 1:
        ...
        for chr_num in range(int(chr_start),int(chr_end)+1):
            cmd = "mv %sAquila_cutPBHC_minicontig_chr%d.fasta %sAquila_Contig_chr%d.fasta"%(in_dir,chr_num,in_dir,chr_num)
            #with open(in_dir+'command.sh','w') as f:
            #    f.write(cmd)
            os.system(cmd)
        ...
        Popen(cmd,shell=True).wait()
        #rename
        cmd = "cd %s;for file in Aquila_*; do mv -v $file ${file/Aquila_/AquilaSV_}; done"%out_dir
        Popen(cmd,shell=True).wait()



        if deletion_mode==1:
            delete_files(args.assembly_dir+"/ref_dir")
            cmd = "find %s -type f ! -name 'AquilaSV_Contig*' -delete;rm %s*gz;"%(out_dir,out_dir)
            Popen(cmd,shell=True).wait()
        #rename_cmd = "mv %s/Aquila_Variant_Results_1bp_allregion %s/AquilaSV_step3_results"%(args.assembly_dir,args.assembly_dir)
        #Popen(rename_cmd,shell=True).wait()
#        for chr_num in range(int(chr_start),int(chr_end)+1):
#            cmd = "cp %sAquila_cutPBHC_minicontig_chr%d.fasta %sAquila_Contig_chr%d.fasta"%(out_dir,chr_num,out_dir,chr_num)
#            os.system(cmd)
