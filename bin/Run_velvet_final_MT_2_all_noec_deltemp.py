...
def split_fastq(fastq_path):
    read1_path=fastq_path.replace('.fastq','_read1.fastq')
    read2_path=fastq_path.replace('.fastq','_read2.fastq')
    with open(fastq_path,'r') as f:
        s=f.read().split('@')[1:]
    seq_list=[read.split('\n')[1] for read in s]
    name_list=[read.split('\n')[0] for read in s]
    read1=[]
    read2=[]
    for i in range(len(seq_list)-1):
        if name_list[i]==name_list[i+1]:
            if len(seq_list[i])==127:
                read1.append('@'+s[i])
                read2.append('@'+s[i+1])
            elif len(seq_list[i])==150:
                read1.append('@'+s[i+1])
                read2.append('@'+s[i])
    ...
    with open(read1_path,'w') as f:
        f.write(read1_s)
    with open(read2_path,'w') as f:
        f.write(read2_s)
...
def use_spades(one_file_fastq,out_dir,xin):
    ...
    split_fastq(one_file_fastq)
    read1_path = one_file_fastq.replace('.fastq','_read1.fastq')
    read2_path = one_file_fastq.replace('.fastq','_read2.fastq')
    cmd_clean = "rm -r %s;"%(out_dir)
    cmd_index = 'velveth %s 77 -short -separate -fastq %s %s ;'%(out_dir,read1_path,read2_path)
    cmd_assemble = 'velvetg %s;'%(out_dir)
    cmd_rename = 'mv %s/contigs.fa %s/contigs.fasta;'%(out_dir,out_dir)
    Popen(cmd_clean+cmd_index+cmd_assemble+cmd_rename,shell=True).wait()
...
def run_spades_all(chr_start,chr_end,output_dir,num_of_threads,minicontig_dir):
    ...
    for chr_num in range(chr_start, chr_end+1):
        ...
        fastq_files_all = [path for path in fastq_files_all if 'read' not in path] # only for main file (not separate)
